...
class ServerConnection(QThread):
    ...
    def __init__(self, parent=None):
        ...
        # Data rate tracker will be set by MainWindow
        self.data_rate_tracker = None
    ...
    def send_msg(self, conn: socket.socket, msg: Message):
        ...
        # Track sent data
        if self.data_rate_tracker:
            self.data_rate_tracker.add_sent_data(
                len(msg_bytes), msg.data_type or "CONTROL"
            )
    ...
    def handle_conn(self, conn: socket.socket, media: str):
        while self.connected:
            ...
            # Track received data
            if self.data_rate_tracker:
                self.data_rate_tracker.add_received_data(len(msg_bytes), media)
